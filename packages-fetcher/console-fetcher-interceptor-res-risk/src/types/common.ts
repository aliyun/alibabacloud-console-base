 ...
 * 旧版主账号风控 - 弹窗的数据类型
 */
export interface IDialogDataOldMainAccountRisk {
 ...
 * 轻量级虚商 - 弹窗类型
 */
export interface IMpkRisk extends Omit<IDialogDataOldMainAccountRisk, 'riskInfo'> {
 ...
 */
export interface IDialogDataNewSubAccountRisk {
  request: FetcherFnRequest;
  subRiskInfo: ISubAccountRiskInfo;
  requestId: string;
  errorMessage: string;
  step?: EStep;
  getBindMfaInfoPayload?: TPayloadBindMfaInfo; // 获取 MFA 绑定信息的接口 /identity/getMfaInfoToBind 的 payload
  getBindMfaInfoData?: TDataBindMfaInfo; // 获取 MFA 绑定信息的接口 /identity/getMfaInfoToBind 的返回 data
  ...
  verifyMfaPayload?: TPayloadVerifyMfa; // 验证 MFA 接口 /identity/verify 的 payload
 ...
 * 主账号风控的弹窗的数据类型
 */
export interface IDialogDataNewMainAccountRisk {
  request: FetcherFnRequest;
