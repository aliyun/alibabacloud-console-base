...
import handleClickGlobal from '../util/handle-click-global'; // 全局劫持就靠这句代码，使用捕获阶段
...
export default function hijackClickGlobal(hijacker) {
  if (!GLOBAL_CLICK_HIJACKERS.includes(hijacker)) {
    // 避免同一个对象多次注册
    GLOBAL_CLICK_HIJACKERS.push(hijacker);
  }

  return function () {
    var index = GLOBAL_CLICK_HIJACKERS.findIndex(function (v) {
      return v === hijacker;
    });
