...
function getMpkSetting(riskInfo: IMpkRiskInfo | IOldMainRiskInfo): IMpkProps {
  const {
    mpkIsDowngrade
  } = riskInfo;
...
export default function Generate(): JSX.Element {
  const {
    ...
    lock,
    unlock
  } = useDialog<void, IMpkRisk | IDialogDataOldMainAccountRisk>();
  ...
  const handleClick = async (): Promise<void> => {
    ...
    const requestHeader = useMpkSendCodeApi ? {
      headers: {
        'Content-Type': 'application/json' // 调用 /identity/send 接口时，Content-Type 要设置为 application/json
      }
    } : {};
    
    try {
      // 这里用当前的 fetcher
      await request<ISendCodeReturn | null>({
      ...
      const errorMessage = (err ? (err as Error).message : '') || '';
