...
import { FetchOptions } from '@alicloud/fetcher-fetch';
import { JsonpOptions } from '@alicloud/fetcher-jsonp';
import { EErrorSpecial } from '../const';
declare type TQuickMethodConfigExclusion = 'url' | 'method';
/**
 * 做一下限制（不推荐手动调用 HEADER/OPTIONS 等），增加 JSONP，理论上大小写无关，但为了保持代码的统一性，这里用全大写
 *
 * https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods
 */
export declare type TMethod = 'GET' | 'POST' | 'DELETE' | 'PUT' | 'PATCH' | 'JSONP';
/**
 * 表示 `() => void` 类型
 */
...
export interface IFetcherConfig extends Omit<FetchOptions, 'method' | 'headers' | 'body'>, JsonpOptions {
     ...
     */
    additionalInterceptorsForRequest?: TArgsForInterceptRequest[];
    /**
     * 调用时临时增加的响应拦截器，不至于影响到整个实例
     */
    additionalInterceptorsForResponse?: TArgsForInterceptResponse[];
}
/**
 ...
 */
export declare type TFetcherOptionsForQuickJsonp<C extends IFetcherConfig = IFetcherConfig> = Omit<C, TQuickMethodConfigExclusion>;
/**
 * 其他便捷方法，如果第一个参数为对象，则为 options
 */
export declare type TFetcherOptionsForQuickFn<C extends IFetcherConfig = IFetcherConfig> = Omit<C, TQuickMethodConfigExclusion | keyof JsonpOptions>;
/**
 * 执行请求的方法定义
 */
...
export declare type TArgsForInterceptRequest<C extends IFetcherConfig = IFetcherConfig> = [IFnInterceptRequest<C>] | [number, IFnInterceptRequest<C>];
export declare type TArgsForInterceptResponse<C extends IFetcherConfig = IFetcherConfig> = [
    IFnInterceptResponseFulfilled<C>?,
    IFnInterceptResponseRejected<C>?
] | [
    number,
    IFnInterceptResponseFulfilled<C>?,
    IFnInterceptResponseRejected<C>?
];
export declare type TArgsForJsonp<C extends IFetcherConfig = IFetcherConfig, P = void> = [string, P?] | [TFetcherOptionsForQuickJsonp<C>, string, P?];
export declare type TArgsForGet<C extends IFetcherConfig = IFetcherConfig, P = void> = [string, P?] | [TFetcherOptionsForQuickFn<C>, string, P?];
export declare type TArgsForPost<C extends IFetcherConfig = IFetcherConfig, B = void, P = void> = [string, B?, P?] | [TFetcherOptionsForQuickFn<C>, string, B?, P?];
export {};
