...
function getEnv() {
  ...
  if (_consoleOneConfig.default.ONE && _consoleOneConfig.default.ENV === 'daily' || /\.test$/.test(hostname) || /daily-|-daily/i.test(hostname)) {
    return _const.EEnv.DAILY;
  }
  /*
   * 预发环境：
   * 
  ...
  if (_consoleOneConfig.default.ONE && _consoleOneConfig.default.ENV === 'pre' || /pre-|-pre/i.test(hostname) || protocol === 'http:') {
    return _const.EEnv.PRE;
  }

  return _const.EEnv.PROD;
}