...
import cookieGetToken from './cookie-get-token'; // FECS 仅支持 .aliyun.com 或其对应日常

var FECS_COMPATIBLE = function () {
  var arr1 = location.hostname.split('.');
  var arr2 = CONF_ENV.FECS_HOST.split('.');
...
function interceptRequest(config) {
  ...
  var fecs = isFecs(config);
  var relativeOne = isRelativeOneApi(config); // 既不走 FECS，也不是当前域名下的 OneConsole API，不需要做什么
  ...
  if (relativeOne) {
    if (ONE_CONF.ONE || !FECS_COMPATIBLE) {
    ...
    return {
      urlBase: "//".concat(CONF_ENV.FECS_HOST),
      body: {
        sec_token: cookieGetToken()
      }
    };
...
export default function createInterceptorReq() {
