...
export interface IMockOptions {
  /**
   * 这里配置的是把匹配到的接口映射到对应的 MOCK 应用下对应的接口，当 url 匹配 RegExp 时，将使用 MOCK_APP 对应的 MOCK 地址
   * 
   * 非 OneConsole 的 mock 地址格式如下：
   * `//oneapi.alibaba-inc.com/mock/` + MOCK_APP + path，一个可运行的例子：
   * https://oneapi.alibaba-inc.com/mock/oss/ajax/kms/list_keys.json
   */
   ...
   * 
   * ```
   * fetch('https://oneapi.alibaba-inc.com/mock/oneconsole/data/api.json', {
   *   method: 'POST',
   ...
   * ```
   * 
   * 而其对应的原 mock 接口是 https://oneapi.alibaba-inc.com/mock/ram_next/CreatePolicy.json
   */
