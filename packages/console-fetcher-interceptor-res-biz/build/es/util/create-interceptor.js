...
export default function createInterceptor() {
  return function (json, config) {
    ...
    var success = isSuccess(json, config.isSuccess);

    if (success) {
      return getData(json, config.getData);
    }

    var code = getCode(json, config.getCode) || '__UNKNOWN__';
    var message = getMessage(json, config.getMessage) || code;
    throw FetcherUtils.createError(config, ERROR_BIZ, message, code);
  };
}